export default function CardParameters({product}) {
  const productYearIndex = product.metafields.findIndex(
    (metafield) => metafield?.key === 'year_production',
  );
  const productYear = product.metafields[productYearIndex].value.split('-');
  const formatedProductYear = productYear[1] + '/' + productYear[0];

  const mileageIndex = product.metafields.findIndex(
    (metafield) => metafield?.key === 'mileage',
  );
  const mileage = product.metafields[mileageIndex].value;
  const formatedMileage = parseInt(mileage, 10).toLocaleString();

  const fuelIndex = product.metafields.findIndex(
    (metafield) => metafield?.key == 'fuel',
  );
  const fuel = fuelIndex > 0 ? product.metafields[fuelIndex].value : '';

  const transmissionIndex = product.metafields.findIndex(
    (metafield) => metafield?.key == 'transmission',
  );
  const transmission =
    transmissionIndex > 0 ? product.metafields[transmissionIndex].value : '';

  const driveIndex = product.metafields.findIndex(
    (metafield) => metafield?.key == 'drive',
  );
  const drive = driveIndex > 0 ? product.metafields[driveIndex].value : '';

  const powerIndex = product.metafields.findIndex(
    (metafield) => metafield?.key == 'power',
  );
